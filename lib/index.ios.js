...
export class SpInAppUpdatesModule {
    ...
    checkNeedsUpdate(checkOptions = {}) {
        ...
        return Siren.performCheck({ version: curVersion }).then((checkResponse) => {
            const info = checkResponse || {};
            const { updateIsAvailable, version } = info;
            const versionCode = updateIsAvailable && version;
            if (versionCode) {
    ...
    startUpdate(defaultOptions, versionSpecificOptions, checkProps) {
        return Promise.resolve(Siren.promptUser(defaultOptions, versionSpecificOptions, checkProps));
    }
